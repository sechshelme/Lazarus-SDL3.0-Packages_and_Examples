...
#define SDL_REQUIRES_SHARED(x) \
  (requires_shared_capability(x))

#define SDL_ACQUIRE(x) \
  (acquire_capability(x))

#define SDL_ACQUIRE_SHARED(x) \
  (acquire_shared_capability(x))

#define SDL_RELEASE(x) \
  (release_capability(x))

#define SDL_RELEASE_SHARED(x) \
  (release_shared_capability(x))

#define SDL_RELEASE_GENERIC(x) \
  (release_generic_capability(x))

#define SDL_TRY_ACQUIRE(x, y) \
  (try_acquire_capability(x, y))

#define SDL_TRY_ACQUIRE_SHARED(x, y) \
  (try_acquire_shared_capability(x, y))
