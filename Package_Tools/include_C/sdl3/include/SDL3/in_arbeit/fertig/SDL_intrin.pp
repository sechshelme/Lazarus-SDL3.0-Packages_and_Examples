...
{
  Automatically converted by H2Pas 1.0.0 from SDL_intrin.h
...
{ __clang__  }
{$include <intrin.h>}
(*** was #elif ****){$else defined(__MINGW64_VERSION_MAJOR)}
{$include <intrin.h>}
...
const
  SDL_NEON_INTRINSICS = 1;  
{$include <arm_neon.h>}
(*** was #elif ****){$else defined(SDL_PLATFORM_WINDOWS)}
...
(*** was #elif ****){$else defined(__clang__) && defined(__has_attribute)}
{$if __has_attribute(target)}
{$define SDL_HAS_TARGET_ATTRIBS}
{$endif}
(*** was #elif ****){$else defined(__GNUC__) && (__GNUC__ + (__GNUC_MINOR__ >= 9) > 4) /* gcc >= 4.9 */}
{$define SDL_HAS_TARGET_ATTRIBS}
(*** was #elif ****){$else defined(__ICC) && __ICC >= 1600}
{$define SDL_HAS_TARGET_ATTRIBS}
{$endif}
{$ifdef SDL_WIKI_DOCUMENTATION_SECTION}
{*
...
function SDL_TARGETING(x : longint) : longint;
...
function SDL_TARGETING(x : longint) : longint;
begin
  SDL_TARGETING:=__attribute__(target(x));
end;
